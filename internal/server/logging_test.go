...
func TestLogChatRowFormat(t *testing.T) {
	...
	out := captureStdout(t, func() {
		logChatRow(412*time.Millisecond, 27100*time.Millisecond, "deepseek-v4-flash", "stream", "00e26541abcdef", http.StatusOK, 1234, UsageInfo{
			PromptTokens: 1000, CompletionTokens: 1234, TotalTokens: 2234,
			CacheHitTokens: 800, CacheMissTokens: 200, Credit: 0.42,
		})
	})
...
func TestLogChatRowNoUsageShowsDash(t *testing.T) {
	withChatLog(t)
	out := captureStdout(t, func() {
		logChatRow(0, time.Second, "glm-5.2", "sync", "s1", http.StatusServiceUnavailable, -1, UsageInfo{})
	})
	for _, want := range []string{"TTFB=-", "out=0", "-tok/s", "| 503 |"} {
		if !strings.Contains(out, want) {
			t.Errorf("row missing %q:\n%s", want, out)
		}
	}
...
func TestLogChatRowSeqIncrements(t *testing.T) {
	withChatLog(t)
	out := captureStdout(t, func() {
		logChatRow(0, time.Second, "m", "sync", "u", 200, 1, UsageInfo{CompletionTokens: 1})
		logChatRow(0, time.Second, "m", "sync", "u", 200, 1, UsageInfo{CompletionTokens: 1})
	})
...
func TestChatLogsStreamRow(t *testing.T) {
	...
	for _, want := range []string{"| stream |", "| 200 |", "uid=u1", "TTFB=", "out=1"} {
		if !strings.Contains(out, want) {
...
func TestChatLogsSyncRowTTFBDash(t *testing.T) {
	...
	for _, want := range []string{"| sync |", "| 200 |", "TTFB=-", "out=1"} {
		if !strings.Contains(out, want) {
...
func TestChatLogsErrorRow(t *testing.T) {
	...
	for _, want := range []string{"uid=u1", "| 503 |", "out=0", "-tok/s"} {
		if !strings.Contains(out, want) {
