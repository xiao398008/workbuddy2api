...
const (
	...
	// modelsForcedMinInterval 强制刷新（?refresh=1）的最短间隔。
	// 计费倍率/促销是随时变的，但上游对高频模型查询会限流，
	// 因此允许绕过 TTL 取实时值，同时用这个下限防止连点把上游打爆。
	modelsForcedMinInterval = 30 * time.Second
	modelsFetchFailCooldown = 5 * time.Minute
)

// models 返回模型列表：优先动态（缓存 1h），失败回退静态表。
// 支持 ?refresh=1 绕过缓存取实时倍率/促销；响应回带 fetched_at 便于前端判断新鲜度。
func (h *Handler) models(w http.ResponseWriter, r *http.Request) {
	force := r.URL.Query().Get("refresh") == "1" || r.URL.Query().Get("refresh") == "true"
	data := h.modelList(force)

	dynamicModelsCache.RLock()
	fetchedAt := dynamicModelsCache.fetched
	dynamicModelsCache.RUnlock()

	resp := map[string]any{
		"object":            "list",
		"data":              data,
		"cache_ttl_seconds": int(dynamicModelsTTL.Seconds()),
	}
	if !fetchedAt.IsZero() {
		resp["fetched_at"] = fetchedAt.Unix()
	}
	writeJSON(w, http.StatusOK, resp)
}

// parseCreditMultiplier 把上游 "x0.03 credits" / "x0.79" 解析成浮点倍率；无法解析返回 nil。
func parseCreditMultiplier(credits string) any {
	if credits == "" {
		return nil
	}
	i := strings.IndexByte(credits, 'x')
	if i < 0 {
		return nil
	}
	s := strings.TrimSpace(credits[i+1:])
	// 取到第一个非数字/非小数点为止（丢弃 " credits" 后缀）
	end := 0
...
// promoTags 只保留 badge:促销名:颜色 里的促销名，展示用。
func promoTags(tags []string) []string {
...
func (h *Handler) modelList(force bool) []map[string]any {
	if infos := h.fetchDynamicModels(force); len(infos) > 0 {
		...
		for _, mi := range infos {
			...
			// 计费倍率与促销标记：上游按「模型×账号」定价，这里原样透出，
			// 让客户端/面板能选到真正打折的模型，而不是只会用贵的。
			entry["credits"] = mi.Credits
			entry["credit_multiplier"] = parseCreditMultiplier(mi.Credits)
			entry["promo_tags"] = promoTags(mi.Tags)
...
// force=true 时绕过 TTL 取实时值（仍受 modelsForcedMinInterval 节流）。
// 拉取失败记录时间戳进入 5min 负缓存，冷却期内直接用静态表，避免反复打上游。
func (h *Handler) fetchDynamicModels(force bool) []upstream.ModelInfo {
	dynamicModelsCache.RLock()
	haveCache := len(dynamicModelsCache.ids) > 0
	age := time.Since(dynamicModelsCache.fetched)
	lastFail := dynamicModelsCache.lastFail

	// 未强制且未过期：命中缓存。
	if haveCache && !force && age < dynamicModelsTTL {
		out := dynamicModelsCache.ids
		dynamicModelsCache.RUnlock()
		return out
	}
	// 强制刷新节流：距上次成功拉取不足 modelsForcedMinInterval 时仍用缓存。
	if haveCache && force && age < modelsForcedMinInterval {
		out := dynamicModelsCache.ids
		dynamicModelsCache.RUnlock()
		return out
	}
	// 失败负缓存：冷却期内不再请求上游（强制刷新同样遵守，避免把正在抖动的上游打挂）。
...
func (h *Handler) chatCompletions(w http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < h.cfg.MaxRotate; i++ {
		...
		writeJSON(w, http.StatusOK, resp)
		st.status = http.StatusOK
		u, comp := parseUsageFromResp(resp)
		st.toks = comp
		st.usage = u
		return
	}
